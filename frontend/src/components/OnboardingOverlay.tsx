  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/20 px-4 py-6 backdrop-blur-sm">
    <section className="mx-auto max-w-4xl animate-[slideDownFade_420ms_ease-out] rounded-3xl border border-slate-200 bg-white shadow-2xl">
      ...
      <div className="max-h-[calc(100vh-12rem)] overflow-y-auto p-5 sm:p-7">
        ...
        {step === 4 && <section><h2 className="text-2xl font-bold text-slate-950">Add a quick financial snapshot</h2><p className="mt-2 text-slate-500">Rough estimates are fine. These numbers update your homepage, not your long-term memory.</p><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><AmountInput label="Monthly income after tax" value={state.snapshot.monthlyIncome} onChange={(value) => updateSnapshot("monthlyIncome", value)} /><AmountInput label="Other monthly income" value={state.snapshot.otherIncome} onChange={(value) => updateSnapshot("otherIncome", value)} /><AmountInput label="Monthly expenses" value={state.snapshot.monthlyExpenses} onChange={(value) => updateSnapshot("monthlyExpenses", value)} /><AmountInput label="Cash savings" value={state.snapshot.cashSavings} onChange={(value) => updateSnapshot("cashSavings", value)} /><AmountInput label="Cash" value={state.snapshot.cash} onChange={(value) => updateSnapshot("cash", value)} /><AmountInput label="Bank savings" value={state.snapshot.bankSavings} onChange={(value) => updateSnapshot("bankSavings", value)} /><AmountInput label="Stocks / ETFs" value={state.snapshot.stocks} onChange={(value) => updateSnapshot("stocks", value)} /><AmountInput label="Superannuation" value={state.snapshot.superannuation} onChange={(value) => updateSnapshot("superannuation", value)} /><AmountInput label="Property" value={state.snapshot.property} onChange={(value) => updateSnapshot("property", value)} /><AmountInput label="Vehicle" value={state.snapshot.vehicle} onChange={(value) => updateSnapshot("vehicle", value)} /><AmountInput label="Other assets" value={state.snapshot.otherAssets} onChange={(value) => updateSnapshot("otherAssets", value)} /></div><div className="mt-6 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4"><p className="text-sm font-bold text-slate-700">Debts / liabilities</p><p className="mt-1 text-sm leading-6 text-slate-500">The current homepage does not have a debt data source yet, so these fields are noted visually for now and can be connected when debt tracking is added.</p><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><AmountInput label="Credit card debt" value={state.snapshot.creditCardDebt} onChange={(value) => updateSnapshot("creditCardDebt", value)} /><AmountInput label="Personal loan" value={state.snapshot.personalLoan} onChange={(value) => updateSnapshot("personalLoan", value)} /><AmountInput label="Student loan" value={state.snapshot.studentLoan} onChange={(value) => updateSnapshot("studentLoan", value)} /><AmountInput label="Mortgage" value={state.snapshot.mortgage} onChange={(value) => updateSnapshot("mortgage", value)} /><AmountInput label="Other debt" value={state.snapshot.otherDebt} onChange={(value) => updateSnapshot("otherDebt", value)} /></div></div></section>}
