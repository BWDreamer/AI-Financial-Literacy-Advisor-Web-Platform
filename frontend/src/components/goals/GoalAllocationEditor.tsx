...
export default function GoalAllocationEditor({ goals, allocations, onApply }: Props) {
  ...
  return (
    <div className="relative">
      ...
      {open && <AllocationPanel error={error} goals={goals} allocations={allocations} drafts={drafts} total={total} setDrafts={setDrafts} onApply={apply} />}
    </div>
  );
}

function AllocationPanel({ error, goals, allocations, drafts, total, setDrafts, onApply }: { error: string; goals: Goal[]; allocations: GoalAllocation[]; drafts: string[]; total: number; setDrafts: React.Dispatch<React.SetStateAction<string[]>>; onApply: () => void }) {
  return (
    <div className="absolute right-0 z-20 mt-2 w-[min(30rem,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
      ...
      <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
        {goals.map((goal, index) => <AllocationRow key={goal.id} goal={goal} index={index} value={drafts[index] ?? "0"} monthlyAmount={allocations[index]?.monthlyAmount ?? 0} setDrafts={setDrafts} />)}
      </div>
...
function AllocationRow({ goal, index, value, monthlyAmount, setDrafts }: { goal: Goal; index: number; value: string; monthlyAmount: number; setDrafts: React.Dispatch<React.SetStateAction<string[]>> }) {
  return (
    <label className="grid grid-cols-[1fr_5.5rem] items-center gap-3 rounded-xl bg-slate-50 p-3">
      <span className="min-w-0">
        ...
        <span className="mt-1 block text-xs font-semibold text-slate-400">Saved: {formatGoalCurrency(monthlyAmount)}</span>
      </span>
      <input
        type="number"
        min="0"
