...
function StatCard({ title, value, stamp, wide = false }: { title: string; value: string; stamp?: string; wide?: boolean }) {
  return <article className={`rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-2xl sm:p-5 ${wide ? "col-span-2 xl:col-span-1" : ""}`}><p className="text-[0.65rem] font-semibold uppercase tracking-wide text-slate-500 sm:text-sm">{title}</p><p className="mt-1 text-xl font-bold leading-tight text-slate-900 sm:mt-3 sm:text-3xl">{value}</p>{stamp && <p className="mt-1 text-[0.65rem] font-semibold text-blue-600 sm:mt-2 sm:text-xs">{stamp}</p>}</article>;
}
...
function RecentCashFlow({ flows }: { flows: CashFlow[] }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-base font-bold text-slate-900 sm:text-lg">Recent Cash Flow</h2><div className="flex rounded-xl bg-slate-100 p-1">{(["month", "week"] as const).map((item) => <button key={item} type="button" onClick={() => changeUnit(item)} className={`rounded-lg px-3 py-1.5 text-xs font-bold capitalize ${unit === item ? "bg-white text-blue-600 shadow-sm" : "text-slate-500"}`}>{item}</button>)}</div></div>{rows.length ? <div className="mt-4 max-h-72 space-y-2 overflow-y-auto pr-1 sm:max-h-96 sm:space-y-3">{visible.map((entry) => <div key={entry.id} className="flex justify-between rounded-xl bg-slate-50 px-3 py-2.5 text-sm sm:px-4 sm:py-3"><div><p className="font-semibold">{entry.name}</p><p className="text-xs text-slate-500 sm:text-sm">{new Date(entry.date).toLocaleDateString("en-AU")}</p></div><p className={entry.flowType === "expense" ? "font-bold text-red-600" : "font-bold text-emerald-600"}>{entry.flowType === "expense" ? "-" : ""}{money(entry.amount)}</p></div>)}</div> : <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No cash flow records for this period.</p>}{totalPages > 1 && <div className="mt-4 flex items-center justify-between text-sm text-slate-500"><button type="button" disabled={!page} onClick={() => setPage(page - 1)} className="rounded-lg px-3 py-2 font-bold disabled:text-slate-300">Previous</button><span>Page {page + 1} of {totalPages}</span><button type="button" disabled={page + 1 >= totalPages} onClick={() => setPage(page + 1)} className="rounded-lg px-3 py-2 font-bold disabled:text-slate-300">Next</button></div>}</section>;
}
...
export default function HomePage() {
  ...
  return <main className="min-h-screen space-y-3 bg-slate-50 p-3 pb-24 sm:space-y-6 sm:p-6 lg:p-8"><header><h1 className="page-title">Insights Overview</h1>{error && <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}</header><section className="grid grid-cols-2 gap-2.5 sm:grid-cols-2 sm:gap-4 xl:grid-cols-5"><StatCard title="Net Worth" value={money(numbers.netWorth)} /><StatCard title="Cash Savings" value={money(numbers.cashSavings)} /><StatCard title="Debt" value={money(numbers.debts)} /><StatCard title="Income" value={money(numbers.income)} stamp={monthLabel()} /><StatCard title="Expenses" value={money(numbers.expenses)} stamp={monthLabel()} wide /></section>
