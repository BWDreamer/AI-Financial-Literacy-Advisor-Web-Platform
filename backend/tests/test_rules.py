...
def create_test_rule(db_session):
    rule = FinancialRule(
        ...
        source_name="Australian Government – Federal Register of Legislation",
...
def test_list_financial_rules(client, db_session):
    ...
    assert data[0]["rule_year"] == "2026-2027"
...
def test_tax_bracket_lookup_returns_marginal_rate_and_citation(
    ...
    assert data["estimated_tax_excluding_medicare"] == 14520.0
    assert data["medicare_levy_included"] is False
    assert data["source_name"] == (
        "Australian Government – Federal Register of Legislation"
    )
    assert data["source_url"] == TAX_RATES_SOURCE_URL
    assert "verified formula" in data["llm_context"]
    assert TAX_RATES_SOURCE_URL in data["llm_context"]
...
def test_superannuation_lookup_returns_rate_year_and_source(
    ...
    assert data["region"] == "Australia"
    assert data["rule_year"] == "2026-2027"
    assert data["period"] == "1 July 2026 – 30 June 2027"
