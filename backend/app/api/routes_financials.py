...
def create_recurring_cash_flow(
    request: RecurringCashFlowRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@router.put(
    "/recurring-cash-flows/{flow_id}",
    response_model=RecurringCashFlowResponse,
)
def update_recurring_cash_flow(
    flow_id: int,
    request: RecurringCashFlowRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    flow = get_recurring_cash_flow(db, current_user.id, flow_id)
    if flow is None:
        raise HTTPException(status_code=404, detail="Recurring cash flow was not found.")
    return save_recurring_cash_flow(db, current_user.id, request, flow)


@router.delete("/recurring-cash-flows/{flow_id}", status_code=status.HTTP_204_NO_CONTENT)
def remove_recurring_cash_flow(
    flow_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    ...
    if flow is None:
        raise HTTPException(status_code=404, detail="Recurring cash flow was not found.")
    delete_recurring_cash_flow(db, flow)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
